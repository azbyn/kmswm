...
public:
    constexpr RectWH(int x, int y, int width, int height) : x(x), y(y), w(width), h(height) {}
...
public:
    constexpr Rect4(int x0, int y0, int x1, int y1) : x0(x0), y0(y0), x1(x1), y1(y1) {}
    constexpr Rect4(Point p0, Point p1) : x0(p0.x), y0(p0.y), x1(p1.x), y1(p1.y) {}
